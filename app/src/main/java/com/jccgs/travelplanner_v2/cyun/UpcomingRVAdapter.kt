...
class UpcomingRVAdapter(val plans: MutableList<Plan>) : RecyclerView.Adapter<UpcomingRVAdapter.CustomViewHolder>(), ItemTouchHelperCallback.OnItemMoveListener {

    private lateinit var dragListener: OnStartDragListener
    lateinit var context: Context

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UpcomingRVAdapter.CustomViewHolder {
        val binding = ItemviewCyunBinding.inflate(LayoutInflater.from(parent.context),parent,false)

        context = parent.context
        val customViewHolder = UpcomingRVAdapter.CustomViewHolder(binding)
    ...
    override fun onBindViewHolder(holder: UpcomingRVAdapter.CustomViewHolder, position: Int) {
        val binding = (holder as UpcomingRVAdapter.CustomViewHolder).binding
